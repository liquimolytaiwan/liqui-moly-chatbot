...
**你必須從「可用產品資料庫」中取得正確的產品資訊：**
1. **產品名稱**：必須使用資料庫中的完整「標題」，不可編造或簡化
2. **產品編號**：必須使用資料庫中的「產品編號」（如 LM21719）
3. **產品連結**：必須使用資料庫中的「產品連結」
4. **認證規格**：必須使用資料庫中的「認證/規格」欄位

**禁止事項：**
- ❌ 禁止編造產品名稱（如把「Molygen Scooter」寫成「Basic Street」）
- ❌ 禁止使用你記憶中的產品資訊
- ❌ 禁止編造連結或使用其他網域

**連結格式**：[產品資料庫中的完整標題](產品資料庫中的連結)
