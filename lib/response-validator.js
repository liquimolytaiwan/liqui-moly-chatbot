...
function extractSKUs(text) {
    ...
    // ⚠️ 過濾可能是年份誤判的 SKU（2019-2030 範圍內的數字）
    // LIQUI MOLY 產品 SKU 通常不會落在這個範圍
    const filteredMatches = matches.filter(sku => {
        const numPart = sku.replace(/[Ll][Mm]/i, '');
        const num = parseInt(numPart, 10);
        // 過濾 2019-2030 範圍內的數字（可能是年份）
        // 同時確保不會誤殺有效 SKU（如 LM20597, LM21494, LM21600 等 5 位數 SKU）
        if (numPart.length === 4 && num >= 2019 && num <= 2030) {
            console.log(`[Validator] Filtered potential year SKU: ${sku}`);
            return false;
        }
        return true;
    });
    ...
    // 標準化為大寫並去重
    const uniqueSKUs = [...new Set(filteredMatches.map(sku => sku.toUpperCase()))];
