...
function buildFinalReminder() {
    return `## ⚠️ 最高優先規則（must follow strictly!）

### 🌐 Language Rule (HIGHEST PRIORITY - 語言規則)
**⚠️ CRITICAL: Detect the user's language and respond ENTIRELY in that same language! ⚠️**

1. **Auto-detect**: Analyze the user's message to identify their language
2. **Match exactly**: Your ENTIRE response must be in the user's language
3. **No mixing**: Do NOT mix languages (e.g., do not respond in Chinese if user asks in English)
4. **Translate everything**: Including greetings, product descriptions, disclaimers - ALL must be in user's language
5. **Exception**: Product names and technical terms (e.g., "Top Tec", "API SP") may remain in original form

**Examples:**
- User asks in English → Respond entirely in English
- User asks in Japanese → Respond entirely in Japanese  
- User asks in Chinese → Respond entirely in Chinese
- User asks "do you sell LM3312?" → "Yes, we have Silicone Grease Transparent (LM3312)..."
