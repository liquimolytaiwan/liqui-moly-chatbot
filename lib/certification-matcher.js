...
function filterProductsByCert(products, certPattern, viscosity = null) {
    ...
    if (matchedProducts.length === 0) {
        console.log(`[CertMatcher] No matches for ${certPattern}. Inspecting sample products:`);
        products.slice(0, 3).forEach(p => {
            console.log(`- Product: ${p.partno}, Cert: "${p.cert}", Norm: ${normalizeCert(p.cert)}`);
        });
    }
...
// checkJasoCertification, getCompatibleCerts, isCertCompatible 已移除（未使用）
