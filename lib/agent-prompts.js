...
function selectAgent(intent) {
    ...
    // 🔧 新增：需追問產品類別時，使用 GREETING agent（會觸發追問邏輯）
    if (intent.needsProductCategoryQuestion) {
        console.log('[selectAgent] Product category question needed, using GREETING agent');
        return AGENT_TYPES.GREETING;
    }
 ...
 * 🔧 v2.1: 支援追問產品類別
 */
function buildGreetingPrompt(intent = null) {
    const { loadJSON } = require('./knowledge-cache');
    const templates = loadJSON('response-templates.json');
    const ecommerceWarning = templates?.purchase_inquiry?.ecommerce_warning;
    const ecommercePlatforms = ecommerceWarning?.keywords?.join('、') || '蝦皮、MOMO、PCHOME、Yahoo、露天';

    // 🔧 新增：處理產品類別追問
    if (intent?.needsProductCategoryQuestion) {
        const vehicleModel = intent?.vehicleModel || intent?._aiAnalysis?.vehicles?.[0]?.vehicleName || '您的車型';
        return `${getLanguageRule()}

${getCoreIdentity()}

## 任務：追問產品類別
用戶提供了車型資訊，但尚未指定要推薦「哪一類」產品（機油、添加劑等）。

**⚠️⚠️⚠️ 極重要：用戶只說了「產品推薦」，沒有說具體是機油還是添加劑！⚠️⚠️⚠️**
...
📌 English Version:
"Hello! Regarding your ${vehicleModel}, what are you looking for?
1. Engine oil
2. Additives (leak stop, cleaning, protection, etc.)
3. Other maintenance products
Please let me know your needs and I'll recommend the best products!"

⚠️ USE THE VERSION MATCHING USER'S LANGUAGE! DO NOT COPY CHINESE IF USER WROTE IN ENGLISH!`;
    }
...
**⚠️ 電商平台警告（極重要！禁止幻覺！）：**
- LIQUI MOLY Taiwan **沒有**官方蝦皮、MOMO、PCHOME 等電商店鋪！
- 禁止編造任何電商連結（如 shopee.tw/xxx）
- 若用戶問電商平台（${ecommercePlatforms}）：
  → 回覆：「${ecommerceWarning?.template || '電商平台非公司貨，無品質保證。建議透過官方管道購買。'}」
- 唯一官方線上商城：CarMall 車魔商城 https://www.carmall.com.tw/（僅部分商品）

**範例（🔴 翻譯成用戶語言！）：**

📌 中文: 「您好！歡迎詢問 LIQUI MOLY 產品！請問您想找引擎機油、添加劑，還是有其他問題呢？」
📌 English: "Hello! Welcome to LIQUI MOLY! Are you looking for engine oil, additives, or do you have other questions?"
...
function buildOilPrompt(knowledge, intent, productContext) {
    ...
    if (typeof rawNeedsInfo === 'string' && rawNeedsInfo.trim()) {
        // 如果是字串，轉為單元素陣列
        needsInfo = [rawNeedsInfo.trim()];
    } else if (Array.isArray(rawNeedsInfo)) {
        needsInfo = rawNeedsInfo;
    }

    // 過濾掉黏度相關的追問（LLM 應該自行推論）
    needsInfo = needsInfo.filter(info => {
        if (typeof info !== 'string') return false;
        const lowerInfo = info.toLowerCase();
        return !lowerInfo.includes('黏度') && !lowerInfo.includes('viscosity');
    });
...
${needsInfo.map((info, idx) => {
            // 智慧判斷：如果已經是問句，就直接顯示，否則加上問候語
            if (info.includes('請問') || info.includes('？') || info.includes('?')) {
                return `${idx + 1}. ${info}`;
            }
            return `${idx + 1}. 請問${info}是什麼呢？`;
        }).join('\n')}
...
function buildAdditivePrompt(knowledge, intent, productContext) {
    ...
    const vehicleInfo = aiAnalysis?.vehicles?.[0];
    ...
    // 判斷是否已知燃油類型（汽油/柴油）- 僅汽車需要
    const isMotorcycle = intent?.isMotorcycle || vehicleInfo?.vehicleType === '機車';
    const hasFuelType = isMotorcycle || !!(
        vehicleInfo?.fuelType ||
        intent?.fuelType
    );

    // 偵測症狀關鍵字
    const symptom = aiAnalysis?.additiveGuideMatch?.detectedSymptom ||
        aiAnalysis?.additiveGuideMatch?.keyword ||
        aiAnalysis?.detectedSymptom ||
        intent?.symptomKeyword ||
        null;
    ...
    // 症狀匹配結果（如有）- 只在有完整車型資訊時顯示
    const additiveMatch = intent?.additiveGuideMatch || aiAnalysis?.additiveGuideMatch;
    if (additiveMatch?.matched && hasVehicleType) {
        sections.push(buildAdditiveMatchSection(additiveMatch));
    }

    // 產品資料庫 - 只在有完整車型資訊時顯示
    if (productContext && hasVehicleType) {
        sections.push(`## 可用產品\n${productContext}`);
    }
    ...
    sections.push(buildFinalReminder(true, {
        ...
        includePurchaseGuide: hasVehicleType
    }));
...
function buildAgentPrompt(agentType, knowledge, intent, productContext) {
    switch (agentType) {
        case AGENT_TYPES.GREETING:
            return buildGreetingPrompt(intent);
