...
function applyKnowledgeToIntent(intent, knowledge) {
    ...
    const specs = knowledge.vehicleSpec;
    if (!Array.isArray(specs) || specs.length === 0) return intent;
    ...
    if (specs.length === 1) {
        // 如果只有一種規格（例如該車型只有汽油版），直接使用
        matchedSpec = specs[0];
    } else if (aiFuelType) {
        // 根據 AI 識別的燃油類型進行匹配
        matchedSpec = specs.find(s => s.fuel && (s.fuel.includes(aiFuelType) || (aiFuelType === '汽油' && !s.fuel.includes('柴油'))));
    }
