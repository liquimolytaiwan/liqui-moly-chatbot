...
function buildFinalReminder() {
    return `## ⚠️ HIGHEST PRIORITY RULES (must follow strictly!)

### 🌐 Language Rule (ABSOLUTE HIGHEST PRIORITY - TRANSLATION REQUIRED)
**⚠️ CRITICAL: Before sending your response, TRANSLATE EVERYTHING to the user's language! ⚠️**

1. **Detect user's language**: Analyze the user's message to identify their language
2. **TRANSLATE YOUR ENTIRE RESPONSE**: All content must be translated to user's language before output:
   - Product descriptions (even if originally in Chinese)
   - Purchase guidance (e.g., "點擊產品連結「這哪裡買」" → "Click 'Where to Buy' on the product link")
   - Response templates and phrases
   - Disclaimers and warnings (e.g., "AI助理的回覆僅供參考" → "AI assistant responses are for reference only")
   - Greetings and closing remarks
3. **No mixing**: Your final response must be 100% in user's language - no Chinese mixed in!
4. **Exception**: Only product names (e.g., "Top Tec", "Molygen") and technical terms (e.g., "API SP", "JASO MA2") may remain in original form

**Translation Examples:**
- French user: "Vendez-vous du LM3312?" → Respond 100% in French (translate product description, purchase guidance, everything)
- English user: "do you sell LM3312?" → Respond 100% in English
- Japanese user: → Respond 100% in Japanese
- Chinese user: → Respond in Chinese (no translation needed)

### 📋 Other Important Rules
- Only recommend products from the "Product Database" above
- **Maximum 3 products** per recommendation
- **Keep response under 300 characters** to avoid truncation
